...
input_location = os.getenv("DATA_INPUT_LOCATION")
output_location = os.getenv("DATA_OUTPUT_LOCATION")
...
def process_image(im_idx):
    ...
    rgb_stack = Image.open(f"{input_location}/{im_idx}.png")
    curr_dead = None
    rgb_scaled = im_rescaler(rgb_stack)
    curr_im, curr_vec = im_vectorizer(rgb_scaled)
    curr_im.save(f"{output_location}/{im_idx}.png")
    np.savetxt(
        f"{output_location}/{im_idx}.txt",
        curr_vec,
    )
    with open(
        f"{output_location}/{im_idx}_deadpix.pkl",
        "wb",
    ) as file:
...
if __name__ == "__main__":
    all_files = os.listdir(f"{input_location}/")
    file_names = [
        f
        for f in all_files
        if os.path.isfile(
    ...
    clean_ids = [x.replace(".png", "") for x in file_names]
