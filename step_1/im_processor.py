...
def process_image(im_idx):
    ...
    curr_im.save(f"/data/beegfs/astro-storage/groups/jwst/fowler/data/images/{im_idx}.png")
    np.savetxt(f"/data/beegfs/astro-storage/groups/jwst/fowler/data/text/{im_idx}.txt", curr_vec)
    with open(f"/data/beegfs/astro-storage/groups/jwst/fowler/data/text/{im_idx}_deadpix.pkl", 'wb') as file:
        pickle.dump(curr_dead, file)

if __name__ == "__main__":
    all_files = os.listdir("/data/beegfs/astro-storage/groups/banados/jwolf/euclid_ero/Perseus_out/fits_cutouts/")
    file_names = [f for f in all_files if os.path.isfile(os.path.join("/data/beegfs/astro-storage/groups/banados/jwolf/euclid_ero/Perseus_out/fits_cutouts/", f))]
